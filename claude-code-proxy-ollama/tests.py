...
def get_response(url, headers, data):
    ...
    # Remove x-api-key if PREFERRED_PROVIDER is ollama and we are sending to proxy
    if PREFERRED_PROVIDER == "ollama" and url == PROXY_API_URL:
        request_headers.pop("x-api-key", None)
        print("Removed x-api-key for Ollama proxy request.")

    start_time = time.time()
    response = httpx.post(url, headers=request_headers, json=data, timeout=30)
...
async def stream_response(url, headers, data, stream_name):
    ...
    if PREFERRED_PROVIDER == "ollama" and url == PROXY_API_URL:
        request_headers.pop("x-api-key", None)
        print("Removed x-api-key for Ollama proxy stream request.")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            start_time = time.time()
            async with client.stream("POST", url, json=request_data, headers=request_headers, timeout=30) as response:
